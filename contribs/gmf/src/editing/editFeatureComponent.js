...
Controller.prototype.continueWithoutSaving = function() {
  if (!this.confirmDeferred_) {
    throw 'Missing confirmDeferred_';
  }
  this.cancel();
  this.confirmDeferred_.resolve();
};
    ...
    this.gmfEditFeature_.deleteFeature(
      ...
      (response) => {
        if (!this.editableWMSLayer_) {
          throw 'Missing editableWMSLayer_';
        }
        this.dirty = false;
        this.pending = false;
...
Controller.prototype.toggle_ = function(active) {
  if (!this.modifyToolActivate) {
    throw 'Missing modifyToolActivate';
  }
  if (!this.translateToolActivate) {
    throw 'Missing translateToolActivate';
  }
  if (!this.rotateToolActivate) {
    throw 'Missing rotateToolActivate';
  }
  if (!this.modify_) {
    throw 'Missing modify';
  }
  if (!this.editableTreeCtrl) {
    throw 'Missing editableTreeCtrl';
  }
  ...
  if (active) {
    if (!this.translate_) {
      throw 'Missing translate';
    }
    if (!this.rotate_) {
      throw 'Missing rotate';
    }
